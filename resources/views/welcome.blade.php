...
<head>
	...
	<meta name="description"
		content="Live market terminal, verified signals, FVG and trendline analysis, financial news and risk management.">
	...
	<style>
		/* ===== Fixed Telegram Community Button ===== */
		.tg-float {
			position: fixed;
			bottom: 24px;
			right: 24px;
			z-index: 9999;
			animation: tgSlideIn 0.6s cubic-bezier(0.16, 1, 0.3, 1) 1.5s both;
		}
		...
		.tg-btn {
			...
			background: linear-gradient(135deg, #0A1020 0%, #0D1526 100%);
		...
		.tg-btn:hover {
			border-color: rgba(41, 169, 234, 0.7);
			transform: translateY(-2px);
			box-shadow: 0 8px 30px rgba(41, 169, 234, 0.25);
		}
		...
		.tg-btn::before {
			...
			inset: -3px;
			border-radius: 99px;
			background: linear-gradient(135deg, rgba(41, 169, 234, 0.4), transparent, rgba(41, 169, 234, 0.2));
			opacity: 0;
		...
		.tg-glow {
			...
			width: 70px;
			height: 70px;
			background: radial-gradient(circle, rgba(41, 169, 234, 0.3) 0%, transparent 70%);
			filter: blur(12px);
			border-radius: 50%;
			animation: tgGlowPulse 2.5s ease-in-out infinite;
		}
		...
		.tg-icon-wrap {
			...
			height: 38px;
			border-radius: 50%;
			background: linear-gradient(135deg, #3390EC, #229ED9);
			display: flex;
			align-items: center;
		...
		.tg-icon-wrap::after {
			...
			inset: -4px;
			border-radius: 50%;
			border: 2px solid rgba(51, 144, 236, 0.25);
			animation: tgIconPing 2s ease-out infinite;
		}

		@keyframes tgIconPing {
			0% {
				transform: scale(1);
				opacity: 0.6;
			}
...
<body class="public-body">
	...
	<header class="public-nav">
		<a class="public-brand" href="{{ route('home') }}"><span
				class="brand-mark">Sdach</span><strong>KOFX</strong></a>
		<nav id="publicNavMenu">
			...
			<a href="#" onclick="document.getElementById('supportModal').style.display='flex'; return false;"
				style="color: var(--green);">Support me ☕</a>
		</nav>
		<div class="nav-actions">
			<button class="theme-toggle" type="button" aria-label="Toggle theme"><span
					class="theme-icon"></span></button>
			@auth
				<a href="{{ route('dashboard') }}" class="public-profile-link"
					style="display:flex; align-items:center; gap:8px; text-decoration:none; color:var(--text); font-weight:600; padding:4px 12px 4px 4px; background:var(--raised); border-radius:999px; border:1px solid var(--border); transition:border-color 0.2s;">
					@if(auth()->user()->avatar)
					...
					@else
						<div
							style="width:28px; height:28px; border-radius:50%; background:linear-gradient(135deg, var(--blue), #7c5cff); color:#fff; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:800; text-transform:uppercase;">
			...
			@for($i = 0; $i < 6; $i++)
				<div class="ticker-set">
					@foreach($markets as $market)
						<span
							class="tick"><b>{{ $market->symbol }}</b><span>{{ number_format($market->price, $market->precision()) }}</span><em
								class="{{ $market->change_pct >= 0 ? 'up' : 'down' }}">{{ $market->change_pct >= 0 ? '▲' : '▼' }}
								{{ number_format(abs($market->change_pct), 2) }}%</em></span>
					@endforeach
				</div>
			@endfor
		</div>
	...
	<main>
		...
		<section class="hero">
				...
				<svg viewBox="0 0 1000 400" preserveAspectRatio="xMidYMid slice">
					...
					<g class="bg-bars">
						@for($i = 0; $i < 60; $i++)
							...
							<rect x="{{ $i * 18 }}" y="{{ $y }}" width="2" height="{{ $h }}" fill="rgba(63, 206, 143, 0.15)">
								<animate attributeName="height" values="{{ $h }};{{ $h + rand(-40, 80) }};{{ $h }}"
									dur="{{ $dur }}s" repeatCount="indefinite" />
								<animate attributeName="y" values="{{ $y }};{{ $y - rand(-40, 80) }};{{ $y }}"
									dur="{{ $dur }}s" repeatCount="indefinite" />
							</rect>
						@endfor
					</g>

					<path class="bg-line" d="M0,350 Q150,280 300,320 T600,200 T1000,100" fill="none" stroke="#3fce8f"
						stroke-width="3" filter="url(#glow)">
						<animate attributeName="d"
							values="M0,350 Q150,280 300,320 T600,200 T1000,100; M0,320 Q150,300 300,280 T600,250 T1000,80; M0,350 Q150,280 300,320 T600,200 T1000,100"
							dur="8s" repeatCount="indefinite" />
					</path>
					<path class="bg-area" d="M0,350 Q150,280 300,320 T600,200 T1000,100 L1000,400 L0,400 Z"
						fill="url(#chartGrad)">
						<animate attributeName="d"
							values="M0,350 Q150,280 300,320 T600,200 T1000,100 L1000,400 L0,400 Z; M0,320 Q150,300 300,280 T600,250 T1000,80 L1000,400 L0,400 Z; M0,350 Q150,280 300,320 T600,200 T1000,100 L1000,400 L0,400 Z"
							dur="8s" repeatCount="indefinite" />
					</path>
					...
					<g transform="translate(800, 80)" filter="url(#glow)">
						<path d="M0,0 L30,-30 L60,0 L40,0 L40,40 L20,40 L20,0 Z" fill="#3fce8f" transform="rotate(30)">
							<animateTransform attributeName="transform" type="translate" values="0,15; 0,-15; 0,15"
								dur="4s" repeatCount="indefinite" additive="sum" />
						</path>
					</g>
					<g transform="translate(300, 260)" filter="url(#glow)">
						<path d="M0,0 L20,-20 L40,0 L25,0 L25,25 L15,25 L15,0 Z" fill="#3fce8f" opacity="0.6"
							transform="rotate(10)">
							<animateTransform attributeName="transform" type="translate" values="0,10; 0,-10; 0,10"
			...
			<div class="hero-copy reveal">
				...
				<p>One professional workspace for live candles, FVG zones, automatic trendlines, verified entry signals,
					news sentiment and disciplined risk.</p>
				<div class="hero-actions">
					@auth
						<a class="btn btn-primary btn-large" href="{{ route('terminal.show', $markets->first()) }}">Launch
							terminal</a>
					@else
				...
				<div class="social-proof-widget">
					<div class="avatars">
						@foreach($recentUsers as $u)
							@if($u->avatar)
								<img src="{{ str_starts_with($u->avatar, 'http') ? $u->avatar : asset('storage/' . $u->avatar) }}"
									alt="{{ $u->name }}">
							@else
			...
			<div class="hero-terminal reveal">
				...
				<div class="mock-head">
					<div>
						<b>{{ $heroMarket?->symbol ?? 'XAUUSD' }}</b><span>{{ $heroMarket?->name ?? 'Gold / US Dollar' }}</span>
					</div>
					<strong
						id="nfxHeroPrice">{{ number_format($heroMarket?->price ?? 0, $heroMarket?->precision() ?? 2) }}</strong>
				...
				<div class="mock-chart">
					<svg viewBox="0 0 700 340" preserveAspectRatio="none">
						<g class="mock-grid">
							<path
								d="M0 55H700M0 120H700M0 185H700M0 250H700M100 0V340M230 0V340M360 0V340M490 0V340M620 0V340" />
						</g>
			...
			<div class="public-market-grid">
				@foreach($markets as $market)
					<article class="public-market reveal">
						...
						<span class="{{ $market->change_pct >= 0 ? 'up' : 'down' }}">{{ $market->change_pct >= 0 ? '▲' : '▼' }}
							{{ number_format(abs($market->change_pct), 2) }}%</span>
						<small
							class="feed-{{ $market->data_status }}">{{ strtoupper($market->data_status) }}{{ $market->data_source === 'yahoo' ? '' : ' · ' . strtoupper($market->data_source) }}</small>
					</article>
				@endforeach
			</div>
		</section>
		...
		<section class="features" id="features">
			...
			<div class="feature-grid">
				<article class="feature-card reveal">
					...
					<p>M1 to D1 candles, candle countdown, OHLC crosshair, volume and automatic refresh without
						reloading.</p>
				</article>
				<article class="feature-card reveal">
					<span>02</span>
					<h3>FVG &amp; structure</h3>
					<p>Bullish/bearish fair value gaps, swing trendlines and volume profile calculated from the active
						timeframe.</p>
				</article>
				<article class="feature-card reveal">
					<span>03</span>
				...
				<article class="feature-card reveal">
					<span>04</span>
					<h3>Risk &amp; journal</h3>
					<p>Position sizing, planned risk, trade review, win rate, net P/L and R-multiple tracking for every
						user.</p>
				</article>
			</div>
		</section>
		...
		<section class="strategies-section" id="strategies">
			<div class="section-head reveal">
				...
				<p>We analyze the market using the most advanced trading concepts to provide high-probability entries.
				</p>
			</div>
							...
							<div class="strategy-media">
								@foreach($strategy->images as $img)
									<img src="{{ str_starts_with($img, 'http') ? $img : asset('storage/' . $img) }}"
										alt="{{ $strategy->title }}" loading="lazy">
								@endforeach
							</div>
						@else
			...
			<div class="public-news-grid">
				@foreach($news as $item)
					<article class="public-news reveal">
						<div>
							<span
								class="badge {{ $item->sentiment === 'bullish' ? 'badge-buy' : ($item->sentiment === 'bearish' ? 'badge-sell' : 'badge-neutral') }}">{{ strtoupper($item->sentiment) }}</span>
	...
	{{-- ===== Fixed Telegram Community Button ===== --}}
	<div class="tg-float">
		<a href="https://t.me/+74M0KgMxstFkYjY1" target="_blank" class="tg-btn">
			...
			<div class="tg-text">
				<span class="tg-text-main">Join community</span>
				<span class="tg-text-sub">Telegram · Free</span>
			</div>
		</a>
	</div>
</body>
