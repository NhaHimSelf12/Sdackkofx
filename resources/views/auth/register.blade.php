...
<div class="auth-brand"><span class="brand-mark">Nha</span><div><strong>Nha FX</strong><div class="muted">Start trading with a process</div></div></div>
<div class="card auth-card"><h1>Create your account</h1><p class="muted">Save your watchlist, risk plans and trading journal.</p>
@if($errors->any())<div class="alert alert-error">{{ $errors->first() }}</div>@endif
<form method="POST" action="{{ route('register.submit') }}" class="form-stack">@csrf
<label>Name<input class="input" name="name" value="{{ old('name') }}" required></label>
<label>Email<input class="input" type="email" name="email" value="{{ old('email') }}" required></label>
...
<p class="auth-switch">Already registered? <a href="{{ route('login') }}">Sign in</a></p></div></div></body></html>
